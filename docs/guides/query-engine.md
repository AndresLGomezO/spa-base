...
See also: [Entity System Guide](./entity-system.md) · [Relational Data System Guide](./relational-data-system.md) · [Entity query definition JSON](../reference/entity-query-definition.md) · [CRUD README](../../apps/api/src/crud/README.md) · Ecosystem plan
...
**Web client (10.2):** `EntityTable` / `EntityCardView` build `QueryConfig` via `@repo/ui-builder` and pass it through `useEntity` → `api-client.listEntity(..., { query })`. See [Advanced UI Builder Guide](./advanced-ui-builder.md).
...
- [Relational Data System Guide](./relational-data-system.md) — FK fields like `customerId` are filterable via the query engine
- [Firestore collections guide](./firestore-collections.md) — tenant-scoped collection paths
