...
**Tenant-scoped business entities** are defined dynamically via Model Builder or statically with `defineEntity()` in optional modules. See [Entity System Guide](./entity-system.md) and [@repo/entities README](../../packages/entities/README.md).
...
Related: [Entity System Guide](./entity-system.md) — tenant-scoped entities defined via `defineEntity()`.
...
- Dev seed on API startup: **none** — run `pnpm seed:database` manually when [`.local/tenant-import/tenant.json`](../../.local/tenant-import/tenant.json) is present (catalogs + records under that folder; see `apps/api` README).
...
For Storage logo uploads, the API also sets `FIREBASE_STORAGE_EMULATOR_PUBLIC_HOST=127.0.0.1:9199` so URLs saved to Firestore are browser-accessible. See [gcs-storage-guide.md](./gcs-storage.md).
...
4. Optional: save a theme preset or palette — Firestore `tenants/{id}.appearance` stores `preset`, `palettes`, `semantics` (see [theme-and-tenant-branding-guide.md](./theme-and-tenant-branding.md)).
...
See [Relational Data System Guide](./relational-data-system.md) for full relation storage and validation behavior.
