...
**Workstream:** 10.4 CUSTOM BUSINESS LOGIC (HOOKS).md>)

**Authoritative Data Hooks spec:** [data-hook-definition-json.md](../reference/data-hook-definition.md)
...
See [data-hook-definition-json.md §2](../reference/data-hook-definition.md#2-execution-model) for full semantics.
...
Entity must exist in the tenant catalog. Full schema: [data-hook-definition-json.md](../reference/data-hook-definition.md).

### Scheduled (time-based) hooks

Hooks with `trigger.kind: "schedule"` register on `{entity}.afterSchedule` and **do not** run on CRUD. Cloud Scheduler (or local curl) POSTs to worker-service `/tasks/schedule-tick` every minute; the worker scans all tenants, finds due cron hooks, and runs them via `runDataHook` using `SCHEDULED_HOOK_USER_UID` as the acting user. See [data-hook-definition-json.md §4](../reference/data-hook-definition.md#4-triggers).
...
- [data-hook-definition-json.md](../reference/data-hook-definition.md) — **full Data Hooks specification**
- [data-hooks.md](./data-hooks.md) — product overview
- [module-extension-guide.md](./module-extension.md)
- [dynamic-entity-builder-guide.md](./dynamic-entity-builder.md)
