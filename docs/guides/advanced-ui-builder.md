...
Design-layout JSON handoff (components, surfaces, presets): [UI Design Manual](../ui-design-manual/README.md).

---

## Unified layout builder (as-built)

There is one recursive layout builder. Forms, lists, dashboards, and screens differ by **composition scope** and **preview context**, not by separate engines.

| Package | Role |
| --- | --- |
| `@repo/ui-builder-core` | Types, Zod schema, composition scopes, grid/container primitives, mutations |
| `@repo/ui-builder-renderer` | Production + preview render (`RecursiveLayoutRenderer`) |
| `@repo/ui-builder-react` | Builder UI, `createLayoutEditorBinding`, structure panel |
| `@repo/ui-builder` | Legacy metadata engine (EntityUIConfig table/form resolution) |

**App integration:** `apps/web/app/features/unified-builder/` (`UnifiedBuilderShell`, preview panel, scope adapters). Design layout routes use the unified shell via `UnifiedDesignerLayoutTab`.

**Persisted shape:** `UiLayoutDocument` with `LayoutRootNode` / `ScreenRootNode`. On load: screen scope → `ensureStandardRoot("screen", layout)`; form surfaces → `ensureFormLayoutGridOnly`; block/component → `ensureContainerRoot`.

**Principles:** preview must match runtime; composition scopes are component / block / section / screen; each builder instance supplies a `PreviewContext` (device switcher only for screen scope).
...
Example validation flow: create `loan` model → open `/app/loan` → table/form render from catalog. Attach hooks via `POST /api/hooks` ([Hooks System Guide](./hooks-system.md)).
...
- [Relational Data System Guide](./relational-data-system.md)
- [Module Extension Guide](./module-extension.md)
- [Dynamic Entity Builder Guide](./dynamic-entity-builder.md)
- [UI Design Manual](../ui-design-manual/README.md)
