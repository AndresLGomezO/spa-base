...
structured entity-to-entity relationships in a schema-driven, multi-tenant environment.

See also: [Entity System Guide](./entity-system.md) · [Firestore collections guide](./firestore-collections.md)
...
- Delete semantics (`restrict`, `nullify`, `cascade`) scoped to the caller's owned records
- Server-side `?populate=` for FK display labels (`_populated`)
- Optional entity `displayField` for human-readable labels

List filtering on FK fields (for example `batchId`) is available via the [Query Engine](./query-engine.md).
...
Used by delete handler and one-to-many table reverse lookup (`useOneToManyColumnData`). The Query Engine uses FK fields for list filters — see [Query Engine Guide](./query-engine.md).
...
See [`firestore.indexes.json`](../../firestore.indexes.json) and [Firestore collections guide](./firestore-collections.md).
...
## Ownership-respecting references

- `validateWrite` accepts optional `userId` and returns `REFERENCE_ACCESS_DENIED` when the caller cannot read the target (skipped for `tenantWideRead` entities).
- Cascade / nullify / restrict in `beforeDelete` only consider records **owned by the caller**; other users' FKs may remain as dangling refs (v1).
- CRUD passes `request.ctx?.uid` into relation validation.

## Populate and display labels

- `GET /api/:entity?populate=field1,field2` (also on get-by-id) — `apps/api/src/access/reference-populator.ts` batch-fetches targets, applies `checkRecordAccess`, and returns labels under `_populated`. Inaccessible targets become `null`.
- Entity `displayField?: string` (Model Builder + static definitions) drives `formatRecordDisplayLabel`; falls back to name/title/label/id.
- Web `useEntity` auto-populates many-to-one / one-to-one columns; detail views link FK values and show reverse one-to-many via `RelatedRecords`.

List filters on FK fields are handled by the [Query Engine](./query-engine.md) (for example `?query={"filter":[{"field":"batchId","operator":"==","value":"…"}]}`).

## Adding relations## Adding relations to a new entity
