...
Reproducible validation for Phase 1 criteria (General Definitions §7) and Phase 2 outcomes (§11). Use this after setup or before handing off to the next team.
...
See [gcs-storage-guide.md](./gcs-storage.md) for Storage emulator and production setup.
...
See [theme-and-tenant-branding-guide.md](./theme-and-tenant-branding.md) for the full token model.
...
Requires Storage emulator in local dev (`FIREBASE_STORAGE_EMULATOR_HOST`). See [gcs-storage-guide.md](./gcs-storage.md).
...
**Layout:** The appearance form is long — confirm only the main content area scrolls (sidebar stays fixed). Confirm only the main content area scrolls (sidebar stays fixed).
