...
## ⚠️ Development context (read first)

**The application is not in production.** There is no live tenant data, no deployed production environment, and no obligation to preserve prior data shapes or behaviors.

When implementing Phase 2 workstreams:

- **Do not** write data migration scripts, backward‑compatibility shims, or legacy code paths for “old” records without ownership fields.
- **Do not** add optional/fallback handling for documents missing `ownerId`, `accessUserIds`, or `sharedWith` — treat ownership and sharing fields as **required system fields from day one** on every new record.
- **Implement greenfield**: integrate features directly into the current platform (DAL, CRUD generator, RBAC, UI) as the canonical behavior going forward.
- **Reset dev data freely** if schema or access rules change; no production rollback or migration plan is required.

**Entity names in this plan are illustrative only.** Examples such as `customer`, `order`, `customer.read_all`, or `order.customer` describe patterns for **any** entity in the registry. **Do not** add those example entities to the repository unless they are already part of your product’s module definitions. Use existing platform entities (e.g. those registered via modules or tenant‑defined dynamic entities) and generic `{entity}` placeholders in tests and docs.
...
> **Greenfield implementation:** Not in production — no migration or legacy paths. Examples like `customer` / `order` are pattern references only; do not create those entities in the repo.
