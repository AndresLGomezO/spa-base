...
interface BackfillLocalMetricsResult {
...
function loadLocalMetricNames(): readonly string[] {
  const parsed = parseMetricDefinitionsCatalogJson(
    loadMetricDefinitionsCatalogJson(),
  );
  if (!parsed.ok) {
    throw new Error(
      `Invalid local metric catalog: ${parsed.errors.map((error) => error.message).join("; ")}`,
    );
  }
  return parsed.data.metricDefinitions.map((metric) => metric.name);
}

export async function activateAndBackfillLocalMetrics(
  tenantId: string,
  firebaseAdminConfig: FirebaseAdminConfig,
  entityRuntime: EntityRuntimeContext,
): Promise<BackfillLocalMetricsResult> {
  ...
  const desiredNames = new Set(loadLocalMetricNames());
  ...
  for (const metric of metrics) {
    try {
      ...
      backfilled += 1;
      console.log(
        `[local-tenant seed] Backfilled "${current.name}" (${current.sourceModel})`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      failures.push({ metricName: metric.name, error: message });
      console.error(
        `[local-tenant seed] Failed to backfill "${metric.name}": ${message}`,
      );
    }
  }
