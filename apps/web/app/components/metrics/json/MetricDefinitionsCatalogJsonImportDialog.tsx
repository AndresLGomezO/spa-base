...
interface MetricDefinitionsCatalogJsonImportDialogProps {
  readonly existingItems: readonly ApiMetricDefinitionRecord[];
...
export function MetricDefinitionsCatalogJsonImportDialog({
        ...
        createMetricDefinitionsCatalogEnvelope([
          {
            ...
            target: { collection: "metric_example", granularity: "dynamic" },
            version: 1,
  ...
  const validation = useMemo(() => {
    ...
    return validateMetricDefinitionsCatalogImport(jsonText);
  }, [jsonText]);
  ...
  const replacePlan = useMemo(() => {
    ...
    return computeCatalogReplacePlan({
      existing: existingItems as readonly MetricDefinitionRecord[],
      imported: validation.data.metricDefinitions,
    });
  ...
  return (
    <>
      ...
      >
        ...
        <Text className="mt-1 text-sm">{labels.backfillSummary}</Text>
      </Modal>
    </>
  );
}
