...
import {
  ...
  SETTINGS_AI_STATEMENT_EXTRACTIONS_NAV_ITEM,
  DEBUGGER_GROUP_ICON,
...
export function useAccessibleNavItems(): readonly NavItemConfig[] {
  ...
  return useMemo(() => {
    ...
    if (
      hasPermission("ai.documentExtract.read", permissions, { isSuperAdmin })
    ) {
      settingsChildren.push(SETTINGS_AI_STATEMENT_EXTRACTIONS_NAV_ITEM);
    }
