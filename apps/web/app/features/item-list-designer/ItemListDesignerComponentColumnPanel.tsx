...
interface ItemListDesignerComponentColumnPanelProps {
  readonly columnRef: ComponentColumnRef;
}

export function ItemListDesignerComponentColumnPanel({
  columnRef,
}: ItemListDesignerComponentColumnPanelProps) {
  ...
  const showGroupedColumnHeaderField =
    ...
    !isNestedComponentColumnRef(columnRef);
  const groupedColumnIndex =
    structureScope.kind === "groupedColumnCell"
      ? structureScope.columnIndex
      : null;

  return (
    <div className="flex flex-col gap-3">
      {showGroupedColumnHeaderField && groupedColumnIndex != null ? (
        <ItemListDesignerGroupedColumnHeaderField
          columnIndex={groupedColumnIndex}
        />
      ) : null}
      {columnCount > 1 ? (
