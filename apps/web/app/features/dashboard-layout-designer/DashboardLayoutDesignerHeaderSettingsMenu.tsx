...
export function DashboardLayoutDesignerHeaderSettingsMenu() {
  const { t } = useTranslation("common");
  const { editor, canSave } = useDashboardLayoutDesigner();
  const exportData = useMemo(
    () => ({
      dashboardSections: editor.dashboardSections,
      dashboardLayout: editor.dashboardLayout,
    }),
    [editor.dashboardLayout, editor.dashboardSections],
  );
  ...
  const importValidation = useMemo(() => {
    ...
    try {
      ...
      const result = putTenantDashboardLayoutInputSchema.safeParse(parsed);
      if (!result.success) {
        return {
          ok: false as const,
  ...
  const handleApplyImport = () => {
    ...
    editor.setDashboardSections(
      importValidation.data.dashboardSections as Parameters<
        typeof editor.setDashboardSections
      >[0],
    );
    editor.setDashboardLayout(
      importValidation.data.dashboardLayout as UiLayoutDocument,
    );
    ...
    <>
      <Popover
        ...
        trigger={
          <IconButton
            type="button"
            label={t("dashboardLayoutDesigner.settings")}
            size="sm"
      ...
      <Modal
        ...
        title={t("dashboardLayoutDesigner.settings")}
      >
