...
export type DashboardLayoutUnsavedReason = "tab" | "structurePanel" | "section";

export type DashboardLayoutPanelTarget =
  | { readonly kind: "row"; readonly rowRef: ComponentRowRef }
  | { readonly kind: "column"; readonly columnRef: ComponentColumnRef };

export type DashboardLayoutPanelPendingAction =
  | { readonly type: "close" }
  | {
      readonly type: "switch";
      readonly target: DashboardLayoutPanelTarget;
      readonly label: string;
    };

export interface DashboardLayoutPanelSession {
  readonly target: DashboardLayoutPanelTarget;
  readonly label: string;
  readonly baseline: UiLayoutDocument;
}

export function areDashboardLayoutPanelTargetsEqual(
  left: DashboardLayoutPanelTarget,
  right: DashboardLayoutPanelTarget,
): boolean {
...
export function isDashboardLayoutPanelSessionDirty(
