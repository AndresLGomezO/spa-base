...
interface CustomViewDefinitionJsonImportDialogProps {
...
export function CustomViewDefinitionJsonImportDialog({
  mode,
  existingViewId,
  existingQueryName,
  canApply,
  ...
  const validation = useMemo(() => {
    ...
    const parsed = validateCustomViewDefinitionImport(jsonText);
    if (!parsed.ok) {
      return parsed;
    }
    ...
    if (
      mode === "edit" &&
      existingViewId &&
      parsed.data.viewId !== existingViewId
    ) {
      return {
        ok: false as const,
        errors: [
          {
            path: "data.viewId",
            message: labels.viewIdChangeError,
          },
        ],
      };
    }
    ...
    if (
      mode === "edit" &&
      existingQueryName &&
      parsed.data.entityQueryDefinitionName !== existingQueryName
    ) {
      return {
        ok: false as const,
        errors: [
          {
            path: "data.entityQueryDefinitionName",
            message: labels.queryNameChangeError,
          },
        ],
      };
    }
    ...
    existingViewId,
    jsonText,
    labels.queryNameChangeError,
    labels.viewIdChangeError,
    mode,
  ]);
