...
interface RecordDetailRootNestedLayout {
  readonly row: NestedLayoutRowNode;
  readonly rootColumnIndex: number;
}

function isCanonicalRecordDetailNestedLayoutRoot(
...
export function resolveRecordDetailRootNestedLayout(
  layout: UiLayoutDocument,
): RecordDetailRootNestedLayout | null {
  if (!isCanonicalRecordDetailNestedLayoutRoot(layout)) {
    return null;
  }
...
export function ensureRecordDetailNestedLayoutRoot(
  layout: UiLayoutDocument,
): UiLayoutDocument {
  const existing = resolveRecordDetailRootNestedLayout(layout);
  if (existing) {
    return layout;
  }
...
export function isRecordDetailRootNestedLayoutRow(
  layout: UiLayoutDocument,
  rowId: string,
): boolean {
  return resolveRecordDetailRootNestedLayout(layout)?.row.id === rowId;
}
