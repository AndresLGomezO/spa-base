...
interface MainPageRootNestedLayout {
  readonly row: NestedLayoutRowNode;
  readonly rootColumnIndex: number;
}

function isCanonicalMainPageNestedLayoutRoot(
...
export function resolveMainPageRootNestedLayout(
  layout: UiLayoutDocument,
): MainPageRootNestedLayout | null {
  if (!isCanonicalMainPageNestedLayoutRoot(layout)) {
    return null;
  }
...
export function ensureMainPageNestedLayoutRoot(
  layout: UiLayoutDocument,
): UiLayoutDocument {
  const existing = resolveMainPageRootNestedLayout(layout);
  if (existing) {
    return layout;
  }
...
export function isMainPageRootNestedLayoutRow(
  layout: UiLayoutDocument,
  rowId: string,
): boolean {
  return resolveMainPageRootNestedLayout(layout)?.row.id === rowId;
}
