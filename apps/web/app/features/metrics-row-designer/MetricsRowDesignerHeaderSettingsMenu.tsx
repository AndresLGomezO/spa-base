...
export function MetricsRowDesignerHeaderSettingsMenu() {
  const { t } = useTranslation("common");
  const { editor, canSave } = useMetricsRowDesigner();
  const definition = useEntityDefinition(editor.entityName);
  const labels = useMemo(
    () => designLayoutSliceJsonLabels(t, "metricsRowDesigner"),
    [t],
  );
    ...
    <>
      <Popover
        ...
        trigger={
          <IconButton
            type="button"
            label={t("metricsRowDesigner.settings")}
            size="sm"
