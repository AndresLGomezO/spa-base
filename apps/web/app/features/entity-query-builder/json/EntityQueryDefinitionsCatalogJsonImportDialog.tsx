...
interface EntityQueryDefinitionsCatalogJsonImportDialogProps {
  ...
  readonly onApply: (catalog: EntityQueryDefinitionsCatalogEnvelope) => void;
...
export function EntityQueryDefinitionsCatalogJsonImportDialog({
  ...
  const validation = useMemo(() => {
    ...
    return validateEntityQueryDefinitionsCatalogImport(jsonText);
  }, [jsonText]);
  ...
  const replacePlan = useMemo(() => {
    ...
    return computeCatalogReplacePlan({
      existing: existingItems as readonly EntityQueryDefinitionRecord[],
      imported: validation.data.entityQueryDefinitions,
    });
