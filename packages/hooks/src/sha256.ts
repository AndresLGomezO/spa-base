/**
 * Synchronous SHA-256 (hex) for isomorphic expression evaluation.
 * Avoids `node:crypto`, which Vite cannot load in browser bundles.
 */
...
export function sha256Hex(text: string): string {
  ...
  const paddedLength = ((withOne + 8 + 63) & ~63) >>> 0;
  const padded = new Uint8Array(paddedLength);
  padded.set(data);
  padded[data.length] = 0x80;

  const view = new DataView(padded.buffer);
  // SHA-256 length is 64-bit big-endian; for practical string sizes the high
  // word stays 0.
