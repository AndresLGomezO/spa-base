...
function taskDescriptionForStep(step: UiBuilderStep): string {
  switch (step.type) {
    ...
    case LIST_STEP_TYPES.LAYOUT_SKELETON: {
      ...
      if (pathKey === "listItem") {
        return "Design a visually rich card listItem skeleton. Use a nested-layout root with 2–3 columns. Lead with image (if an image/file field exists in allowed paths) or icon in the first column; place bold title text, colorful badge status, and date/numeric metadata in remaining slots. Do not produce a flat text-only skeleton.";
      }
