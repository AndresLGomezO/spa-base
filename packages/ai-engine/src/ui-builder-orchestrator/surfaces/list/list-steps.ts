...
export const selectViewTypeOutputSchema = z
  .object({
    listViewType: z.enum(["card", "expandableTable"]),
  })
  .strict();
...
export const STEP_OUTPUT_INSTRUCTIONS: Record<ListStepType, string> = {
  [LIST_STEP_TYPES.SELECT_VIEW_TYPE]: `Return ONLY compact JSON: { "listViewType": "card" | "expandableTable" }. Prefer "card" when the entity has image/logo fields or the user wants visual, mobile-first, premium UX. Default to "expandableTable" for standard entity lists.`,
