...
export const STEP_OUTPUT_INSTRUCTIONS: Record<ListStepType, string> = {
  ...
  [LIST_STEP_TYPES.LAYOUT_SKELETON]: `Return ONLY compact JSON skeleton: { "components": [...] }. Each item is either { "kind": "text"|"image"|"icon"|"date"|"numeric"|"badge"|"metric-kpi", "fieldPath": string, "displayFrom"?, "displayTo"? } or { "kind": "nested-layout", "columnCount": number, "columns": [{ "components": [...] }] }. Do NOT return root/listItem/full layout documents or component props like primary/label/id/styles. For listItem card layouts: use one nested-layout root with 2–3 columns; **lead column with image (if field exists) or icon**; mix badge/date/numeric — avoid a skeleton of text-only rows.`,
