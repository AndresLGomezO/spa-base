# Total Balance gradient KPI card
...
5. **Chart overlay (last sibling)** — `chart` with `chartType: "area"`, `displayMode: "overlay"`, and `dataSource.type: "entityQuery"` with **`layout: "monthToDateRightAligned"`** (daily buckets 1→N left to right through calendar today when the dashboard month matches the current month; full past months capped at 30). Use `Transaction trend` with total-balance `rowFilters` / `valueTransforms` (expense, payment, and investment rows × `-1`). KPI still reads **`Total Balance by Month`** via metric bindings; the overlay shows daily net transaction totals month-to-date. Place the chart row **after** header and body.
